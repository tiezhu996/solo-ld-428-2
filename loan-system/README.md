...
借展单模块自带一套零依赖的端到端回归测试（通过真实 HTTP 接口验证，而非内部函数直调）：

```bash
python3 loan-system/tests/test_loan_regression.py        # 汇总输出
python3 loan-system/tests/test_loan_regression.py -v     # 每条用例明细
```

测试进程会在系统临时目录中用**随机端口 + 独立 SQLite 文件**启动一份真实服务，seed 数据按
当天日期动态生成，运行结束自动销毁，退出码全通过为 0、有失败为 1（可直接接入 CI），
不会改动 `server/data/` 下的本机库。覆盖：

- A. 已归还历史借展窗口重叠时同一作品可再次出借（含 `dry_run` 不落库）
- B. 与 pending / active（含逾期）借展重叠一律 409；区间首尾相接不算重叠；混合作品一件冲突整单阻止
- C. 创建/编辑时重复作品编号返回 400 且明确指出重复项，库中数据与原关联保持不变（无 5xx）
- D. 取消待出库借展后档期立即释放可再借；active 借展不可取消
- E. 出库 → 到馆 → 逐件归还核对 → 结项完整状态流转；跳序/缺件/重复交接拦截；未核对不可结项；结项后锁定
- F. 逾期 / 临期 / 在途异常提醒在流转后仍正确
